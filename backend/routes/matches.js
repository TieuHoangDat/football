...
router.get("/competitions", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      if (err.code === 'ER_BAD_FIELD_ERROR' && err.sqlMessage && err.sqlMessage.includes('logo_url')) {
        const fallbackQuery = `
        ...
        db.query(fallbackQuery, (fallbackErr, fallbackResults) => {
          if (fallbackErr) {
            console.error("Lỗi truy vấn thay thế danh sách giải đấu:", fallbackErr);
            return res.status(500).json({ error: "Lỗi khi lấy danh sách giải đấu" });
          }
...
router.get("/seasons", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy danh sách mùa giải" });
    }
...
router.get("/filter", (req, res) => {
  ...
  db.query(query, params, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy danh sách trận đấu theo bộ lọc" });
    }
    res.json(results);
  });
});
...
router.get("/recent", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy danh sách trận đấu gần đây" });
    }
    res.json(results);
  });
});
...
router.get("/stats/:matchId", (req, res) => {
  ...
  db.query(query, [matchId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy thống kê trận đấu" });
    }
    res.json(results);
  });
});
...
router.get("/players/:matchId", (req, res) => {
  ...
  db.query(query, [matchId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy danh sách cầu thủ tham gia trận đấu" });
    }
    res.json(results);
  });
});
...
router.get("/player-stats/:matchId/:playerId", (req, res) => {
  ...
  db.query(query, [matchId, playerId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Lỗi khi lấy thống kê cầu thủ" });
    }

    if (results.length === 0) {
      return res.status(404).json({ error: "Không tìm thấy thông tin thống kê" });
    }
